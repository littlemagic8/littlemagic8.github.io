---
layout:     post
title:      一分钟搞定 SuperGrok 充值，SuperGrok【代充值】自助订阅手把手教程
subtitle:   卡密 + Grok ID，国内支付自助开通 SuperGrok / Heavy
date:       2026-08-14
author:     aicygg888
header-img: img/post-bg-cook.jpg
catalog: true
tags:
    - SuperGrok代充
    - SuperGrok Heavy
---

## 准备工作

1. **Grok ID**：先登录官网 [grok.com](https://grok.com)；ID 获取地址：[https://grok.com/api/auth/session](https://grok.com/api/auth/session)
2. **卡密**：SuperGrok 自助充值系统所需的卡密（需要卡代充联系：**aicygg888** / **aicygg789**）

下面是获取卡密和 Grok ID 的详细图文步骤。

---

## 第一步：准备卡密

> **tips：卡密获取地址**：[https://littlemagic8.github.io/gptplus/purchase-grok.html](https://littlemagic8.github.io/gptplus/purchase-grok.html)

![购买 SuperGrok 卡密](/img/2026-08-14-how-to-sub-supergrok/image-20260816095734091.png)

![选择 SuperGrok 档位](/img/2026-08-14-how-to-sub-supergrok/image-20260816095952134.png)

购买成功后，系统自动发货（**包含卡密和充值系统，以每次系统发货的充值系统为准**）：

![购买成功后系统自动发货，包含卡密和充值系统](/img/2026-08-14-how-to-sub-supergrok/image-20260816100555676.png)

准备好卡密和充值系统之后，进入第二步。

---

## 第二步：准备 Grok ID

登录 [grok.com](https://grok.com) → 新建标签页，打开：[https://grok.com/api/auth/session](https://grok.com/api/auth/session)

> **注意：必须和已经登录的 grok.com 在同一个浏览器，否则网页显示为空。**

![复制 grok.com session 里的 userId](/img/2026-08-14-how-to-sub-supergrok/grok-userid.png)

---

## 第三步：打开充值系统，充值 SuperGrok

![打开充值系统](/img/2026-08-14-how-to-sub-supergrok/image-20260816101503701.png)

![填写卡密和 Grok ID](/img/2026-08-14-how-to-sub-supergrok/image-20260816101519419.png)

![确认并升级](/img/2026-08-14-how-to-sub-supergrok/image-20260816101531083.png)

---

## 第四步：检查订阅是否到账

检查路径：**头像 → 设置 → 账号**

> **tips：在账单里能看到更多订阅信息。**

![设置里查看订阅](/img/2026-08-14-how-to-sub-supergrok/image-20260816101447691.png)

![账单里确认 SuperGrok 到账](/img/2026-08-14-how-to-sub-supergrok/image-20260816102126613.png)

到这里订阅就全部结束了，可以去用 SuperGrok 的全部权益。生图、做视频，再搭配 Grok Build 写文章，体验很不错。

> **小提示：** 想用更多 AI 产品（ChatGPT Plus、Claude、Gemini、SuperGrok 等），可以联系微信：**aicygg888** / **aicygg789**。
>
> 其它代充 AI 服务见：[https://littlemagic8.github.io/gptplus/](https://littlemagic8.github.io/gptplus/)
>
> 同类教程：[ChatGPT Plus](https://littlemagic8.github.io/2026/08/16/how-to-sub-gptplus/) · [Claude Pro](https://littlemagic8.github.io/2026/08/16/how-to-sub-claudepro/) · [X Premium](https://littlemagic8.github.io/2026/08/16/how-to-sub-x/) · [X Premium+](https://littlemagic8.github.io/2026/08/16/how-to-sub-x-premium-plus/)

---

## 联系我们

充值过程中遇到任何问题，或有其他业务需求，请联系我们。

请保存网址 [https://littlemagic8.github.io/gptplus/](https://littlemagic8.github.io/gptplus/) ，并加上联系方式，防止失联。

防失联客服微信：

```
aicygg888
```

添加好友请备注：**GPT代充**（加的人多，微信防频繁，请扫码）

## 小提示：想用更多 AI 产品，可以联系 V: aicygg888

> **如果不想开通信用卡，可参考：无需开通信用卡完成 ChatGPT 订阅教程。**
>
> ChatGPTplus 独享账号升级充值自助平台：[https://littlemagic8.github.io/gptplus/](https://littlemagic8.github.io/gptplus/)（网站下方有购买卡密和使用教程）
>
> 开通自己的 ChatGPT Plus、Claude Pro 个人独享账号，可参考：[使用支付方式订阅开通 ChatGPT Plus、Claude Pro 教程](https://littlemagic8.github.io/2024/09/04/update-ChatGPT-Plus/)
>
> 国内使用 ChatGPT / Claude 镜像账号，有两种方式：

**方式一：按教程自行购买**

遇到问题联系微信：aicygg888  
登录地址：https://chatshare.biz/ （复制到浏览器打开，用购买成功后的账号密码登录）  
购买地址：https://littlemagic8.github.io/buychat/

- [ChatGPT Plus 独享账号教程](https://littlemagic8.github.io/2025/07/17/chatgptplus-auto-system/)
- [Claude Pro 订阅指南](https://littlemagic8.github.io/2024/12/09/ChatGPT-and-Cluade/)
- [镜像账号使用说明](https://littlemagic8.github.io/2025/07/17/chatgptplus-chatshare/)

**方式二：不想自己注册，添加微信购买**

微信：**aicygg888**（备注：镜像账号）

欢迎加微信：

![微信客服](/img/2026-08-02-x-premium-plus/wechat-qr.png)

公众号也可以哦：

![公众号](/img/v2-4e622b64238b20948a02e0c988ca5704_720w.png)
